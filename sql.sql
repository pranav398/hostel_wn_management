...
ALTER TABLE `user` CHANGE `rn` `rn` VARCHAR(4) NOT NULL;
ALTER TABLE `user` CHANGE `contact` `contact` VARCHAR(10) NOT NULL;
CREATE TABLE `hmanwm`.`update` (`id` INT NOT NULL AUTO_INCREMENT , `roll` VARCHAR(7) NOT NULL , `name` VARCHAR(255) NOT NULL , `contact` VARCHAR(10) NOT NULL , `hn` VARCHAR(3) NOT NULL , `rn` VARCHAR(4) NOT NULL , PRIMARY KEY (`id`)) ENGINE = InnoDB;
ALTER TABLE `update` ADD `active` TINYINT(1) NOT NULL DEFAULT '1' AFTER `rn`;
ALTER TABLE `update` ADD `time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER `active`;
